class DashboardComponents {
  ...
  function DashboardComponents(report, metaData, log) {
    _report = report;
    _metaData = metaData;
    _log = log;
  ...
  private static function AddStatisticsColumnHeader(table) {
    ...
    statistics.Statistics.Count = true;
  ...
  function AddCountLabel(chart, dashboardChartNumber) {
    if(_numberOfDashboardQuestions > dashboardChartNumber - 1) {
      var question = _dashboardQuestions[dashboardChartNumber-1];
      if(question.QuestionType == QuestionType.Single || (question.QuestionType == QuestionType.Grid && question.Answers.length == 1)) {
        var tableName = "DashboardChart" + dashboardChartNumber + "Table";
        var count = _report.TableUtils.GetCellValue(tableName, 1, IndexBaseType.FromEnd, 1, IndexBaseType.FromStart, DatapointType.Count).Value;
        var countLabel = "Count: " + count;
        chart.ChartArea.PrimaryAxisY.Title.Text = countLabel;
      }         
    }
  }
  
  function AddChartTitleOnExport(chart, state, chartNumber) {
    if(state.ReportExecutionMode == ReportExecutionMode.PowerPointExport || state.ReportExecutionMode == ReportExecutionMode.ExcelExport) {
      var inExport = true;
      chart.Title.Text = CreateDashboardChartTitle(chartNumber, inExport);
    }
  }
}